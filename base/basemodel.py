...
class BaseModel:
    ...
    @abstractmethod
    def _create_cnn(self, inputs, batch_norm = None ): raise Exception("The method ""_create_cnn"" must be implemented")
    
    @abstractmethod
    def _create_fully_connected(self, batch_norm = False): raise Exception("The method ""_create_fully_connected"" must be implemented")
    ...
    def add(self, layer, *args, **kwargs):
        ...
        if not issubclass(layer, BaseLayer):
            raise Exception("This layer must be a ""BaseModel"" subclass rather than ""{}""".format(type(layer)))
    ...
    def get_output(self):
        return None if self._layers is None else self._layers[-1]
    ...
    def build(self, input_images = None, labels = None, is_tf = True, new_fc = True, istraining = False):
            ...
            if input_images is None or labels is None:   
...
#             self.add(layers.Flatten, name="flatten")
    ...
    def generate_optimization_parameters(self):
        print("Creating optimizer components")
...
#                 loss = tf.reduce_mean(tf.losses.softmax_cross_entropy(logits=logits, onehot_labels=self.dict_model["labels"], weights = class_weights))
                ...
                if self.hparams.regularizer_type:
                    loss = self.regularize(cost, self.hparams.regularizer_type, self.hparams.regularizer_scale)
...
#                     model_path = "{}/{}".format(self._ckpt_dir, self.info["weights"])
...
#                 for epoch in range(self.hparams.num_epochs):
                    ...
                    [model["optimizer"], model["loss"], model["metrics"]["accuracy"], 
...
#                 self._saver.save(self.sess,ckpt_path, global_step=step)        
            ...
            for epoch in range(self.hparams.num_epochs):
                ...
                for s in range(steps_per_epoch):
                    ...
                    feed_dict={input_data_placeholder: batch_data, 
                               model["labels"]: batch_labels,
                               model["keep"]:self.hparams.keep, 
                               self.dict_model["istraining"]:True}
...
#                     print(acc, best_accuracy)
                    if acc > best_accuracy:
                        self._saver.save(self.sess, ckpt_path,  global_step=step)
                        if best_accuracy > 0:
                            print("The model was saved. The current evaluation accuracy ({:.2f}%) "\
                                  "is greater than the last one saved({:.2f}%).".format(acc*100,best_accuracy*100) 
                                 )
                        else:
                            print("The model was saved.")
                        best_accuracy= acc
...
#         self.plot_data()
    ...
    def _evaluate(self, data, labels, batch_size = 96):
        ...
        if self.sess is None:
            self.sess = self.create_monitored_session(self.dict_model)
    ...
    def test_prediction(self, batch_size = 96):
        ...
        if self._istraining or not self.built:
            ...
            self.build()
        if self.sess is None:
            self.sess,_ = self.create_monitored_session(self.dict_model)
...
#         print(data.shape, labels.shape)
        ...
        for i in range(size+1):
            ...
            if self.hparams.data_augmentation and not self.hparams.auto_crop == (0,0): 
                prob = self._pred_with_crop(input_data_placeholder, batch_xs, self.hparams.auto_crop[0], self.hparams.auto_crop[1])
            else:
...
#         print(mes.format( acc, sum(correct), len(data)))
        ...
        mes = "--> Precision: {:.2f}% -> {} \n--> Recall: {:.2f}% -> {}\n--> F1-score: {:.2f}% -> {}\n--> Accuracy: {:.2f}%\n\n"
        print(mes.format(
             100*np.mean(p),["{:.2f}".format(v*100) for v in p],
             100*np.mean(r), ["{:.2f}".format(v*100) for v in r],
             100*np.mean(f), ["{:.2f}".format(v*100) for v in f],
             acc 
                        )
             )
        plt.bar(np.arange(self.hparams.num_classes),s)
        tick_marks = np.arange(self.hparams.num_classes)
        plt.xticks(tick_marks, self.hparams.class_names, rotation=45)
#         plt.yticks(tick_marks, self.hparams.class_names)
        plt.ylabel('Number of Samples')
        plt.xlabel('Classes')

        return predictions

    def _pred_with_crop(self, input_data_placeholder, batch_xs, crop_h, crop_w):
        logits = self._layers[-1].output
        
        batchs = [batch_xs[:,:-crop_h,:-crop_w,:], 
                  batch_xs[:,crop_h:,:-crop_w,:], 
                  batch_xs[:,crop_h:,crop_w:,:], 
                  batch_xs[:,:-crop_h,crop_w:,:]] 
